...
from configuration_deepseek import DeepseekConfig, deepseek_1b_config
from losses import DiversityLoss
...
if is_flash_attn_2_available():
    ...
    print("Flash attention is not available. Please install the flash_attention package to use it.")
...
class MoEGate(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if self.training: self.logits = logits
...
class DeepseekMoE(nn.Module):
    ...
    # def forward(self, hidden_states, residual=None, labels=None):
    ...
    def forward(self, hidden_states, random_routing: Optional[str] = None):
        identity = hidden_states
        orig_shape = hidden_states.shape
        topk_idx, topk_weight, aux_loss = self.gate(hidden_states)
        hidden_states = hidden_states.view(-1, hidden_states.shape[-1])
        ...
        if self.training:
            ...
            if self.config.diversity_loss:
                diversity_loss = DiversityLoss.calc(self.experts) * self.config.beta
                y = AddAuxiliaryLoss.apply(y, diversity_loss)

        else:
            if random_routing == 'one_expert':
                topk_idx_one_expert = self.random_change_indices(topk_idx, self.config.n_routed_experts - 1)
                y = self.moe_infer(hidden_states, topk_idx_one_expert.view(-1),
                                   topk_weight.view(-1, 1)).view(*orig_shape)
            elif random_routing == 'random_experts':
                y = self.moe_infer(hidden_states,
                                   torch.randint(0, self.config.n_routed_experts, (hidden_states.shape[0],),
                                                 device=hidden_states.device), topk_weight.view(-1, 1)).view(
                    *orig_shape)
            elif random_routing == 'random_weighted':
                random_weight = torch.rand_like(topk_weight)
                random_weight = random_weight / random_weight.sum(dim=1, keepdim=True)
                y = self.moe_infer(hidden_states, flat_topk_idx, random_weight.view(-1, 1)).view(
                    *orig_shape)
            else:
                y = self.moe_infer(hidden_states, flat_topk_idx, topk_weight.view(-1, 1)).view(*orig_shape)
...
class DeepseekDecoderLayer(nn.Module):
    ...
    def forward(
            ...
            random_routing: Optional[str] = None,
            **kwargs,
        ...
        if isinstance(self.mlp, DeepseekMoE):
            hidden_states = self.mlp(hidden_states, random_routing=random_routing)
        else:
...
class DeepseekModel(DeepseekPreTrainedModel):
    ...
    def forward(
            ...
            decoder_layer_idx: Optional[int] = None,
            random_routing: Optional[str] = None,
        ...
        for i, decoder_layer in enumerate(self.layers):
            if decoder_layer_idx is not None and decoder_layer_idx > i:
                continue
...
class DualOptimizerDeepseekLightningModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # if batch_idx % 50 == 0:
        gain_loss = self.compute_additional_loss(outputs, batch)
        # else: gain_loss =  -1 # Full forward pass
        # self.manual_backward(outputs.loss)#, retain_graph=True)
        # opt1.step()
        # opt1.zero_grad()
        # self.untoggle_optimizer(opt1)

        # Compute loss2 and update model2
        # self.toggle_optimizer(opt2)
        # gain_loss = self.compute_additional_loss(outputs, batch)  # Full forward pass
        ...
        self.log("Gain Loss", gain_loss, prog_bar=True, sync_dist=True)
        return outputs.loss
    ...
    def compute_additional_loss(self, outputs, batch):
        ...
        for i in range(1, len(outputs.hidden_states) - 1):
            ...
            batch_new.update(
                {
                    "inputs_embeds": outputs.hidden_states[i].detach(),
                    # Fix: Detach to avoid modifying computation graph
                    "decoder_layer_idx": i,
        ...
        return torch.stack(gain_losses).mean() if gain_losses else torch.tensor(-1., device=self.device)
...
class InterpretCallback(pl.Callback):
    def __init__(self, interpret_steps=10):
        super().__init__()
        self.interpret_steps = interpret_steps

    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        if (trainer.global_step) % self.interpret_steps == 0 and trainer.is_global_zero:
            self.interpret(trainer, pl_module, batch, batch_idx)

    @rank_zero_only
    def interpret(self, trainer, pl_module, batch, batch_idx):
        text = "An attention function can be described as mapping a query and a set of key-value pairs to an output, where the query, keys, values, and output are all vectors. The output is"
        inputs = pl_module.tokenizer(text, return_tensors="pt")
        outputs = pl_module.model.generate(**inputs.to(pl_module.model.device), max_new_tokens=100)

        result = pl_module.tokenizer.decode(outputs[0], skip_special_tokens=True)
        print(result)


def run_pytorch_lightning_trainer(model, test_tokenized_dataset, tokenizer, train_tokenized_dataset,
                                  output_path, lr, batch_size, epochs):
    ...
    # lightning_model = DeepseekLightningModule(model, tokenizer, lr)
    # lightning_model = DualOptimizerDeepseekLightningModule(model, tokenizer, lr)
    optimizer = torch.optim.AdamW(model.parameters(), lr=lr)
    lightning_model = LightningModule(model, tokenizer, optimizers=optimizer,
                                      additional_loss_function=None)
    # additional_loss_function=compute_additional_loss)

    data_module = LLMDataModule(train_tokenized_dataset, test_tokenized_dataset, tokenizer, batch_size)
    ...
    trainer = Trainer(
        ...
        callbacks=[checkpoint_callback, InterpretCallback(interpret_steps=10000 // batch_size)],
    )

    # Train the model
    trainer.fit(lightning_model,
                datamodule=data_module)  # , ckpt_path='/data/users/buznahy/deepseek/results/deepseek_gain_loss_batch_32/2025_02_12_00_27_26/best-checkpoint.ckpt')

    # Evaluate the model
    eval_results = trainer.validate(lightning_model, datamodule=data_module)
    print(f"Average evaluation loss: {eval_results[0]['val_loss']}")
    return model


def run_huggingface_trainer(output_dir, model, test_tokenized_dataset, tokenizer, train_tokenized_dataset, lr,
...
def train_vanilla_pytorch(output_dir, model, test_tokenized_dataset, tokenizer, train_tokenized_dataset, epochs=20,
                          lr=0.1, batch_size=1):
    ...
    # Create DataLoaders
    train_dataloader = DataLoader(train_tokenized_dataset, batch_size=batch_size, collate_fn=data_collator)
    test_dataloader = DataLoader(test_tokenized_dataset, batch_size=batch_size, collate_fn=data_collator)
    ...
    for epoch in range(epochs):
        total_loss = 0
        for batch in (pbar := tqdm(train_dataloader, desc=f"Epoch {epoch + 1}/{epochs}")):
            ...
            for i in range(1, len(outputs.hidden_states) - 1):
                batch_new = batch.copy()
                batch_new.update({"inputs_embeds": outputs.hidden_states[i], "decoder_layer_idx": i,
                                  "past_key_values": DynamicCache(), 'use_cache': False,
                                  'random_routing': 'random_weighted'})
                del batch_new["input_ids"]
                with torch.no_grad():
                    model.eval()
            ...
            model.train()
            loss = outputs.loss  # - outputs_new.loss
            # mean_loss = loss.mean().item()
            # remove the negative losses
            ...
            # pbar.set_postfix_str(f"len of positive losses: {len([l for l in losses if l > 0])} / {len(losses)}")
            pbar.set_postfix_str(f"Mean loss: {np.mean(losses)}, Loss: {loss.item()}")
        avg_train_loss = total_loss / len(train_dataloader)
        print(f"Epoch {epoch + 1}/{epochs}, Training Loss: {avg_train_loss}")
        print(f"len of positive losses: {len([l for l in losses if l > 0])} / {len(losses)}")
...
def main():
    ...
    test_open_orca_ds = load_dataset("Open-Orca/OpenOrca", split="train[:10000]",
                                     cache_dir=f"{base_path}/datasets/test_10000_length_1024")
    train_open_orca_ds = load_dataset("Open-Orca/OpenOrca", split="train[10000:]"
                                      , cache_dir=f"{base_path}/datasets/test_length_1024")
    ...
    test_tokenized_dataset = test_open_orca_ds.map(lambda x:
                                                   ...
                                                   cache_file_name=f'{base_path}deepseek/cache/test_1000_length_1024').remove_columns(
    ...
    train_tokenized_dataset = train_open_orca_ds.map(lambda x:
                                                     ...
                                                     cache_file_name=f'{base_path}deepseek/cache/train_10000_length_1024').remove_columns(
        ["id", "question", "response", "system_prompt"])
    ...
    # model = DeepseekForCausalLM.from_pretrained(model_name, torch_dtype=torch.bfloat16)

    model = DeepseekForCausalLM(
        config=deepseek_1b_config)  # .to(device), torch_dtype=torch.bfloat16, device_map="auto")
    #
    # model.generation_config.pad_token_id = model.generation_config.eos_token_id
    # tokenizer = AutoTokenizer.from_pretrained("Qwen/Qwen2.5-1.5B")
    # model = AutoModelForCausalLM.from_pretrained("Qwen/Qwen2.5-1.5B")

    experiment_name = "deepseek_gain_loss_batch_32"
    # experiment_name = "qwen"

    output_path = f"{base_path}/deepseek/results/{experiment_name}/{datetime.now().strftime('%Y_%m_%d_%H_%M_%S')}"
    ...
    train(output_path, model, test_tokenized_dataset, tokenizer, train_tokenized_dataset,
          batch_size=32, lr=0.001, epochs=2, mode='lightning')


def train(output_path, model, test_tokenized_dataset, tokenizer, train_tokenized_dataset,
          batch_size=16, lr=0.001, epochs=5, mode='lightning'):
    if mode == 'lightning':
        model = run_pytorch_lightning_trainer(model, test_tokenized_dataset, tokenizer, train_tokenized_dataset,
                                              output_path, lr, batch_size, epochs)
    elif mode == 'huggingface':
        model = run_huggingface_trainer(output_path, model, test_tokenized_dataset, tokenizer,
                                        train_tokenized_dataset, lr, batch_size, epochs)
    else:
        model = train_vanilla_pytorch(output_path, model, test_tokenized_dataset, tokenizer, train_tokenized_dataset,
